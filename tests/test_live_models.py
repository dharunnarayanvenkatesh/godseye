...
not found in .God's Eye/credentials.json.
...
_cred_store = CredentialStore(workspace=_PROJECT_ROOT, session_root_dir=".God's Eye")
...
class OpenRouterLiveTests(unittest.TestCase):
    ...
    def test_openrouter_returns_valid_model_turn(self) -> None:
        model = OpenAICompatibleModel(
            ...
            strict_tools=False,
            extra_headers={
                "HTTP-Referer": "https://github.com/God's Eye",
                "X-Title": "God's Eye",
            },
        )
        conv = model.create_conversation(
