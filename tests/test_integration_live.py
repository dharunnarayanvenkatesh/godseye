...
_cred_store = CredentialStore(workspace=_PROJECT_ROOT, session_root_dir=".godseye")
...
def _read_events(root: Path, session_id: str) -> list[dict]:
    events_path = root / ".godseye" / "sessions" / session_id / "events.jsonl"
    events = []
...
def _read_state(root: Path, session_id: str) -> dict:
    state_path = root / ".godseye" / "sessions" / session_id / "state.json"
...
class OpenAIEventLogIntegration(unittest.TestCase):
    ...
    def test_event_log_complete(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            session_dir = root / ".godseye" / "sessions" / "oai-events"
