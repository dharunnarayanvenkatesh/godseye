...
class CredentialsFromEnvTests(unittest.TestCase):
    ...
    def test_rlm_prefix_takes_priority(self) -> None:
        env = {
            "GODSEYE_OPENAI_API_KEY": "rlm-key",
    ...
    def test_godseye_openai_key_from_env(self) -> None:
        env = {"GODSEYE_OPENAI_API_KEY": "test-key"}
        with patch.dict(os.environ, env, clear=True):
            creds = credentials_from_env()
...
class AgentConfigFromEnvTests(unittest.TestCase):
    ...
    def test_custom_env_overrides(self) -> None:
        env = {
            "GODSEYE_PROVIDER": "anthropic",
            "GODSEYE_MODEL": "claude-opus-4-6",
            "GODSEYE_REASONING_EFFORT": "low",
            "GODSEYE_MAX_DEPTH": "5",
            "GODSEYE_MAX_STEPS": "20",
            "GODSEYE_SHELL": "/bin/bash",
        }
        with patch.dict(os.environ, env, clear=True):
            cfg = AgentConfig.from_env("/tmp/test-ws")
