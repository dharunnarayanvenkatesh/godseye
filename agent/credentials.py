...
class CredentialBundle:
    ...
    def has_any(self) -> bool:
        return bool(
            ...
            or (self.deepseek_api_key and self.deepseek_api_key.strip())
    ...
    def merge_missing(self, other: "CredentialBundle") -> None:
        ...
        if not self.deepseek_api_key and other.deepseek_api_key:
    ...
    def to_json(self) -> dict[str, str]:
        ...
        if self.deepseek_api_key:
            out["deepseek_api_key"] = self.deepseek_api_key
        if self.exa_api_key:
    ...
    def from_json(cls, payload: dict[str, str] | None) -> "CredentialBundle":
        ...
        return cls(
            ...
            deepseek_api_key=(payload.get("deepseek_api_key") or "").strip() or None,
...
def parse_env_file(path: Path) -> CredentialBundle:
    ...
    return CredentialBundle(
        openai_api_key=(env.get("OPENAI_API_KEY") or env.get("GODSEYE_OPENAI_API_KEY") or "").strip() or None,
        anthropic_api_key=(env.get("ANTHROPIC_API_KEY") or env.get("GODSEYE_ANTHROPIC_API_KEY") or "").strip()
        or None,
        openrouter_api_key=(env.get("OPENROUTER_API_KEY") or env.get("GODSEYE_OPENROUTER_API_KEY") or "").strip()
        or None,
        cerebras_api_key=(env.get("CEREBRAS_API_KEY") or env.get("GODSEYE_CEREBRAS_API_KEY") or "").strip()
        or None,
        deepseek_api_key=(env.get("DEEPSEEK_API_KEY") or env.get("GODSEYE_DEEPSEEK_API_KEY") or "").strip()
        or None,
        exa_api_key=(env.get("EXA_API_KEY") or env.get("GODSEYE_EXA_API_KEY") or "").strip() or None,
        voyage_api_key=(env.get("VOYAGE_API_KEY") or env.get("GODSEYE_VOYAGE_API_KEY") or "").strip() or None,
    )
...
def credentials_from_env() -> CredentialBundle:
    return CredentialBundle(
        openai_api_key=(
            os.getenv("GODSEYE_OPENAI_API_KEY")
            or os.getenv("OPENAI_API_KEY")
            or ""
        ).strip()
        or None,
        anthropic_api_key=(
            os.getenv("GODSEYE_ANTHROPIC_API_KEY") or os.getenv("ANTHROPIC_API_KEY") or ""
        ).strip()
        or None,
        openrouter_api_key=(
            os.getenv("GODSEYE_OPENROUTER_API_KEY") or os.getenv("OPENROUTER_API_KEY") or ""
        ).strip()
        or None,
        cerebras_api_key=(
            os.getenv("GODSEYE_CEREBRAS_API_KEY") or os.getenv("CEREBRAS_API_KEY") or ""
        ).strip()
        or None,
        deepseek_api_key=(
            os.getenv("GODSEYE_DEEPSEEK_API_KEY") or os.getenv("DEEPSEEK_API_KEY") or ""
        ).strip()
        or None,
        exa_api_key=(os.getenv("GODSEYE_EXA_API_KEY") or os.getenv("EXA_API_KEY") or "").strip() or None,
        voyage_api_key=(os.getenv("GODSEYE_VOYAGE_API_KEY") or os.getenv("VOYAGE_API_KEY") or "").strip() or None,
    )
...
_USER_CONFIG_DIR = Path.home() / ".godseye"


@dataclass(slots=True)
class UserCredentialStore:
    """User-level credential store at ~/.godseye/credentials.json."""
    credentials_path: Path = field(init=False)
...
def prompt_for_credentials(
    ...
    current.deepseek_api_key = _ask("DeepSeek", current.deepseek_api_key)
