...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="godseye-agent",
        description="God's Eye investigation agent with terminal UI.",
    )
    parser.add_argument("--workspace", default=".", help="Workspace root directory.")
    parser.add_argument(
        "--provider",
        default=None,
        choices=["auto", "openai", "anthropic", "openrouter", "cerebras", "deepseek", "ollama", "all"],
    ...
    parser.add_argument(
        "--default-model",
        help="Persist workspace default model in .godseye/settings.json.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Persist workspace default model for DeepSeek provider.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--deepseek-api-key", help="DeepSeek API key override.")
    ...
    parser.add_argument(
        "--list-sessions",
        action="store_true",
        help="List known sessions in .godseye and exit.",
    )
    parser.add_argument(
        "--list-models",
        action="store_true",
...
def _resolve_provider(requested: str, creds: CredentialBundle) -> str:
    ...
    if requested in {"openai", "anthropic", "openrouter", "cerebras", "deepseek", "ollama"}:
        return requested
    if requested == "all":
        return "all"
...
def _print_models(cfg: AgentConfig, requested_provider: str) -> int:
    ...
    if requested_provider == "all":
        providers = ["openai", "anthropic", "openrouter", "cerebras", "deepseek", "ollama"]
...
def _load_credentials(
    ...
    creds = CredentialBundle(
        ...
        deepseek_api_key=user_creds.deepseek_api_key,
    ...
    if stored.deepseek_api_key:
        creds.deepseek_api_key = stored.deepseek_api_key
    ...
    if env_creds.deepseek_api_key:
        creds.deepseek_api_key = env_creds.deepseek_api_key
    ...
    if args.deepseek_api_key:
        creds.deepseek_api_key = args.deepseek_api_key.strip() or creds.deepseek_api_key
    if args.exa_api_key:
...
def _apply_runtime_overrides(cfg: AgentConfig, args: argparse.Namespace, creds: CredentialBundle) -> None:
    ...
    cfg.deepseek_api_key = creds.deepseek_api_key
    ...
    if args.base_url:
        if cfg.provider == "openai":
            ...
            cfg.deepseek_base_url = args.base_url
...
def run_plain_repl(ctx: ChatContext) -> None:
    ...
    _out("God's Eye Agent (plain mode). Type /quit to exit.")
    while True:
        try:
...
def _apply_persistent_settings(
    ...
    if args.default_model_cerebras is not None:
        ...
        changed = True
    if args.default_model_deepseek is not None:
        settings.default_model_deepseek = args.default_model_deepseek.strip() or None
        changed = True
    ...
    if changed:
        ...
        print("Saved persistent defaults to .godseye/settings.json")

    if (
        args.model is None
        and not os.getenv("GODSEYE_MODEL")
        and settings.default_model
    ):
        cfg.model = settings.default_model
    if (
        args.reasoning_effort is None
        and not os.getenv("GODSEYE_REASONING_EFFORT")
...
def _print_settings(settings: PersistentSettings) -> None:
    ...
    print(f"  default_model_deepseek: {settings.default_model_deepseek or '(unset)'}")
...
def main() -> None:
    ...
    if args.provider and args.model is None and not os.getenv("GODSEYE_MODEL"):
        cfg.model = (
            settings.default_model_for_provider(cfg.provider)
            or PROVIDER_DEFAULT_MODELS.get(cfg.provider, cfg.model)
        )
    ...
    try:
        ...
        if args.textual:
            print("Textual TUI requires extra dependencies: pip install gods-eye-agent[textual]")
            raise SystemExit(1)
