...
class RLMEngine:
    ...
    def _apply_tool_call(
        ...
        if name == "search_sources":
            ...
            return False, self.tools.search_sources(query=query, category=category, limit=limit)

        if name == "source_details":
            source_name = str(args.get("name", "")).strip()
            if not source_name:
                return False, "source_details requires name"
            return False, self.tools.source_details(source_name)
    ...
    def _list_artifacts(self) -> str:
        ...
        artifacts_dir = self.config.workspace / ".godseye_artifacts"
