...
def _build_splash() -> str:
    """Generate the startup ASCII art banner."""
    try:
        import pyfiglet
        art = pyfiglet.figlet_format("God's Eye", font="standard").rstrip()
    except Exception:
        art = "   God's Eye"
...
def handle_model_command(args: str, ctx: ChatContext) -> list[str]:
    ...
    if parts[0] == "list":
        ...
        elif list_target in {"openai", "anthropic", "openrouter", "cerebras", "deepseek", "ollama"}:
    ...
    if save:
        ...
        if provider == "openai":
            ...
            settings.default_model_deepseek = new_model
