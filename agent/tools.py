...
from .source_catalog import categories as source_categories
from .source_catalog import get_source, search_catalog
...
class WorkspaceTools:
    ...
    def search_sources(
        ...
        """Search curated investigative sources before using general web search."""
        bounded_limit = max(1, min(int(limit), 25))
        matches = search_catalog(query, category=category)[:bounded_limit]
        payload = {
            "query": query,
            "category": category,
            "count": len(matches),
            "available_categories": list(source_categories()),
            "sources": [entry.as_dict() for entry in matches],
        }
        return json.dumps(payload, indent=2, ensure_ascii=True)

    def source_details(self, name: str) -> str:
        """Return metadata and related leads for one catalog source."""
        entry = get_source(name)
        if entry is None:
            suggestions = search_catalog(name)[:5]
            return json.dumps(
                {
        ...
        return json.dumps(entry.as_dict(), indent=2, ensure_ascii=True)
