...
/// Central configuration for the God's Eye agent.
///
...
impl AgentConfig {
    ...
    pub fn from_env(workspace: impl AsRef<Path>) -> Self {
        ...
        let openai_api_key = env_opt("GODSEYE_OPENAI_API_KEY")
            .or_else(|| env_opt("OPENAI_API_KEY"));

        let anthropic_api_key = env_opt("GODSEYE_ANTHROPIC_API_KEY")
            .or_else(|| env_opt("ANTHROPIC_API_KEY"));

        let openrouter_api_key = env_opt("GODSEYE_OPENROUTER_API_KEY")
            .or_else(|| env_opt("OPENROUTER_API_KEY"));

        let cerebras_api_key = env_opt("GODSEYE_CEREBRAS_API_KEY")
            .or_else(|| env_opt("CEREBRAS_API_KEY"));

        let deepseek_api_key = env_opt("GODSEYE_DEEPSEEK_API_KEY")
            .or_else(|| env_opt("DEEPSEEK_API_KEY"));

        let exa_api_key = env_opt("GODSEYE_EXA_API_KEY")
            .or_else(|| env_opt("EXA_API_KEY"));

        let voyage_api_key = env_opt("GODSEYE_VOYAGE_API_KEY")
            .or_else(|| env_opt("VOYAGE_API_KEY"));

        let openai_base_url = env_opt("GODSEYE_OPENAI_BASE_URL")
            .or_else(|| env_opt("GODSEYE_BASE_URL"))
            .unwrap_or_else(|| "https://api.openai.com/v1".into());

        let reasoning_effort_raw = env_or("GODSEYE_REASONING_EFFORT", "high")
            .trim()
            .to_lowercase();
        ...
        let provider_raw = env_or("GODSEYE_PROVIDER", "auto")
            .trim()
            .to_lowercase();
        ...
        Self {
            workspace: ws,
            provider,
            model: env_or("GODSEYE_MODEL", "claude-opus-4-6"),
            reasoning_effort,
            base_url: openai_base_url.clone(),
            api_key: openai_api_key.clone(),
            openai_base_url,
            anthropic_base_url: env_or(
                "GODSEYE_ANTHROPIC_BASE_URL",
                "https://api.anthropic.com/v1",
            ),
            openrouter_base_url: env_or(
                "GODSEYE_OPENROUTER_BASE_URL",
                "https://openrouter.ai/api/v1",
            ),
            cerebras_base_url: env_or(
                "GODSEYE_CEREBRAS_BASE_URL",
                "https://api.cerebras.ai/v1",
            ),
            deepseek_base_url: env_or(
                "GODSEYE_DEEPSEEK_BASE_URL",
                "https://api.deepseek.com/v1",
            ),
            ollama_base_url: env_or(
                "GODSEYE_OLLAMA_BASE_URL",
                "http://localhost:11434/v1",
            ),
            exa_base_url: env_or("GODSEYE_EXA_BASE_URL", "https://api.exa.ai"),
            openai_api_key,
            anthropic_api_key,
            openrouter_api_key,
            cerebras_api_key,
...
mod tests {
    ...
    fn test_from_env_defaults_and_custom() {
        let keys = [
            "GODSEYE_PROVIDER",
            "GODSEYE_MODEL",
            "GODSEYE_REASONING_EFFORT",
            "GODSEYE_OPENAI_API_KEY",
            "OPENAI_API_KEY",
            "GODSEYE_ANTHROPIC_API_KEY",
            "ANTHROPIC_API_KEY",
            "GODSEYE_DEEPSEEK_API_KEY",
            "DEEPSEEK_API_KEY",
            "GODSEYE_MAX_DEPTH",
            "GODSEYE_RECURSIVE",
            "GODSEYE_DEMO",
        ...
        unsafe {
            ...
            env::set_var("GODSEYE_PROVIDER", "openai");
            env::set_var("GODSEYE_MODEL", "gpt-5.2");
            env::set_var("GODSEYE_REASONING_EFFORT", "low");
            env::set_var("GODSEYE_MAX_DEPTH", "8");
            env::set_var("GODSEYE_RECURSIVE", "false");
            env::set_var("GODSEYE_DEMO", "true");
