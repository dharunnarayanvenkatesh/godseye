    ...
    match provider {
        "anthropic" => {
            let key = cfg
                ...
                .ok_or_else(|| {
                    ModelError::Message(
                        "No Anthropic API key. Set ANTHROPIC_API_KEY or GODSEYE_ANTHROPIC_API_KEY.".into(),
                    )
                })?;
        ...
        "openai" => {
            let key = cfg
                ...
                .ok_or_else(|| {
                    ModelError::Message(
                        "No OpenAI API key. Set OPENAI_API_KEY or GODSEYE_OPENAI_API_KEY.".into(),
                    )
                })?;
        ...
        "openrouter" => {
            let key = cfg
                ...
                .ok_or_else(|| {
                    ModelError::Message(
                        "No OpenRouter API key. Set OPENROUTER_API_KEY or GODSEYE_OPENROUTER_API_KEY.".into(),
                    )
                })?;
        ...
        "cerebras" => {
            let key = cfg
                ...
                .ok_or_else(|| {
                    ModelError::Message(
                        "No Cerebras API key. Set CEREBRAS_API_KEY or GODSEYE_CEREBRAS_API_KEY.".into(),
                    )
                })?;
        ...
        "deepseek" => {
            let key = cfg
                ...
                .ok_or_else(|| {
                    ModelError::Message(
                        "No DeepSeek API key. Set DEEPSEEK_API_KEY or GODSEYE_DEEPSEEK_API_KEY.".into(),
                    )
                })?;
            Ok((cfg.deepseek_base_url.clone(), key.to_string()))
        }
        "ollama" => {
...
pub fn build_model(cfg: &AgentConfig) -> Result<Box<dyn BaseModel>, ModelError> {
    ...
    match provider.as_str() {
        ...
        _ => {
            // OpenAI-compatible: openai, openrouter, cerebras, deepseek, ollama
            let mut extra_headers = HashMap::new();
            if provider == "openrouter" {
                extra_headers.insert(
                    "HTTP-Referer".to_string(),
                    "https://github.com/godseye".to_string(),
                );
                extra_headers.insert("X-Title".to_string(), "God's Eye".to_string());
            }
...
mod tests {
    ...
    #[test]
    fn test_resolve_endpoint_deepseek() {
        let cfg = AgentConfig {
            deepseek_api_key: Some("sk-deepseek".into()),
            ..Default::default()
        };
        let (url, key) = resolve_endpoint(&cfg, "deepseek").unwrap();
