...
pub const SYSTEM_PROMPT_BASE: &str = r#"You are God's Eye, an analysis and investigation agent operating through a terminal session.
...
== DATA SOURCES WIKI ==
A runtime wiki of data source documentation is available at .God's Eye/wiki/.
Read .God's Eye/wiki/index.md at the start of any investigation to see what
...
relevant entry or create a new one using .God's Eye/wiki/template.md.

=== MANDATORY WIKI INDEXING ===
For EVERY investigation, you MUST maintain the wiki as a living knowledge map:

1. READ .God's Eye/wiki/index.md BEFORE starting any investigation to
...
2. CREATE a wiki entry for EVERY data source you access or discover during the
   investigation, using .God's Eye/wiki/template.md as the template. No
   source should go undocumented.
3. UPDATE .God's Eye/wiki/index.md to link each new entry in the appropriate
   category table.
...
mod tests {
    ...
    fn test_build_system_prompt_base_only() {
        ...
        assert!(prompt.contains("You are God's Eye"));
    ...
    fn test_prompt_section_ordering() {
        let prompt = build_system_prompt(true, true, true);
        let base_pos = prompt.find("You are God's Eye").unwrap();
