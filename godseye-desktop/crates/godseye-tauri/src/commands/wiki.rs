...
use godseye_core::events::{GraphData, GraphEdge, GraphNode, NodeType};
...
/// Checks both `.godseye/wiki/` (preferred) and `wiki/` at each level.
fn find_wiki_dir(start: &Path) -> Option<PathBuf> {
    let mut dir = start.canonicalize().ok();
    while let Some(d) = dir {
        // Prefer .godseye/wiki/ (standard location used by the agent)
        let dot_wiki = d.join(".godseye").join("wiki");
    ...
    for line in content.lines() {
        ...
        if let Some(caps) = LINK_RE.captures(line) {
            ...
            nodes.push(GraphNode {
                ...
                node_type: Some(godseye_core::events::NodeType::Source),
                parent_id: None,
                content: None,
            });
        }
...
| SEC Data | UK | [b](b.md) |";
    ...
    #[test]
    fn test_find_wiki_dir_dot_godseye() {
        let tmp = tempdir().unwrap();
        let wiki = tmp.path().join(".godseye").join("wiki");
        fs::create_dir_all(&wiki).unwrap();
        fs::write(wiki.join("index.md"), "# Index").unwrap();
    ...
    #[test]
    fn test_find_wiki_dir_dot_godseye_preferred_over_bare() {
        let tmp = tempdir().unwrap();
        // Create both wiki/ and .godseye/wiki/
        let bare = tmp.path().join("wiki");
        fs::create_dir_all(&bare).unwrap();
        fs::write(bare.join("index.md"), "# Bare").unwrap();

        let dot = tmp.path().join(".godseye").join("wiki");
    ...
    #[test]
    fn test_find_wiki_dir_dot_godseye_from_child() {
        let tmp = tempdir().unwrap();
        let wiki = tmp.path().join(".godseye").join("wiki");
        fs::create_dir_all(&wiki).unwrap();
        fs::write(wiki.join("index.md"), "# Index").unwrap();

        // Start from a subdirectory — should still walk up and find .godseye/wiki/
    ...
    #[test]
    fn test_dot_godseye_wiki_end_to_end() {
        let tmp = tempdir().unwrap();
        let wiki = tmp.path().join(".godseye").join("wiki");
        fs::create_dir_all(&wiki).unwrap();
        ...
        // project_root should be .godseye/ so joining with wiki/fec.md works
        let project_root = found.parent().unwrap();
        let file_path = project_root.join(&nodes[0].path);
        assert!(file_path.exists(), "should resolve to .godseye/wiki/fec.md");
    }
