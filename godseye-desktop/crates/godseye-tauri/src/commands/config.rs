...
use godseye_core::events::{ConfigView, ModelInfo, PartialConfig};
use godseye_core::settings::{PersistentSettings, SettingsStore};
use godseye_core::credentials::credentials_from_env;
...
pub async fn list_models(
    ...
    if provider == "all" {
        let mut all = Vec::new();
        for p in &["openai", "anthropic", "openrouter", "cerebras", "deepseek", "ollama"] {
...
pub fn build_credential_status(cfg: &godseye_core::config::AgentConfig) -> HashMap<String, bool> {
    ...
    status.insert("deepseek".to_string(), cfg.deepseek_api_key.is_some());
...
pub async fn get_credentials_status(
    ...
    status.insert(
        "deepseek".to_string(),
        cfg.deepseek_api_key.is_some() || env_creds.deepseek_api_key.is_some(),
    );
    status.insert("ollama".to_string(), true); // Ollama never needs a key
    status.insert(
        "exa".to_string(),
...
mod tests {
    ...
    #[test]
    fn test_deepseek_models_nonempty() {
        let models = known_models_for_provider("deepseek");
        assert!(!models.is_empty(), "deepseek should have known models");
    }
    ...
    fn test_model_info_fields() {
        for provider in &["openai", "anthropic", "openrouter", "cerebras", "deepseek", "ollama"] {
    ...
    fn test_cred_status_all_none() {
        let cfg = godseye_core::config::AgentConfig::from_env("/nonexistent");
        ...
        assert_eq!(status["deepseek"], false);
        assert_eq!(status["ollama"], true, "ollama always true");
    }

    #[test]
    fn test_cred_status_openai_set() {
        let mut cfg = godseye_core::config::AgentConfig::from_env("/nonexistent");
    ...
    fn test_cred_status_all_set() {
        ...
        cfg.deepseek_api_key = Some("k5".to_string());
